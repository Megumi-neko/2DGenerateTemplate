...
- 基础亮度、基础 DPS、边缘柔化和发光开关与主蜡烛同步；质量升级会提高这份基础 DPS。
- 内圈始终把聚光倍率锁为 1。站在内圈里时，外圈只叠基础伤害，不会把聚光倍率带进近战区。
...
Stage 1 的 `Main Camera` 会由 `StageLightingCameraFramer` 自动取景。它保持相机原有的旋转角度，只平滑调整位置：圆形光照以中央蜡烛为中心，扇形光照仅进行很小的水平偏移；默认镜头优先保持地图可读性，只有光照范围超出安全视口时才自动后移，抬升被限制在较小范围内；取景器同时计算主扇形和固定圆形内圈的联合边界，因此最窄扇形时蜡烛底部的圆形安全区也不会被裁掉。镜头偏移以蜡烛为基准并带方向中心偏移限制、屏幕安全边距、变化阈值和最大移动限制，避免旋转时晃动或逐帧漂移。透视相机使用安全视口的半宽/半高计算后移距离；范围升级很多次后也会按最新边界重新求解。平滑移动过程中还会执行安全校正，避免新目标尚未追上时短暂裁切扇形或内圈。

原型阶段可以使用以下快捷键测试中央蜡烛升级：

- `1`：范围升级，增加基础半径；扇形的等面积有效射程也会随之增加。
- `2`：强度升级，同时提高光照强度与每秒伤害。

代码或后续 UI 可以直接调用 `StageLightingBootstrap.UpgradeRange()` 和 `StageLightingBootstrap.UpgradeIntensity()`。两类升级等级彼此独立，达到 Inspector 中配置的最大等级后不会继续增加。升级会立即通知相机重新取景；强度升级不会改变光照几何范围，但会保持视觉亮度和伤害同步。
